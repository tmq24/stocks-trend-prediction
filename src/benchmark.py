...
def plot_predictions(ticker: str, predictions: np.ndarray, targets: np.ndarray,
    ...
    """Plot predicted vs actual close price for a ticker and save to model-specific folder."""
    
    # Tạo subfolder theo tên model
    model_plot_dir = os.path.join(PLOT_DIR, model_type)
    ...
    ax.plot(x, targets, label='Actual', linewidth=1.8, color='blue')
    ax.plot(x, predictions, label='Predicted', linewidth=1.8, color='orange')
    ...
    ax.set_title(f'{model_type.upper()} | {ticker} | Window={window_size}d, Horizon={horizon}d')
    ax.legend()
    ...
    # Tên file giữ nguyên
    save_name = f"pred_{ticker}_w{window_size}_h{horizon}.png"  # Đổi thứ tự cho dễ đọc hơn (ticker trước)
    save_path = os.path.join(model_plot_dir, save_name)
